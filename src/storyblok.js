...
export default class Storyblok {
  ...
  getFieldType(component_name, field_name) {
    ...
    if (!component) {
      console.error(
        `The component ${component_name} doesn't exist in your Storyblok Space. The field will be imported as plain text`
      );
      return "text";
    }
    ...
    try {
      ...
      console.error(
        `The field ${field_name} doesn't exist in the schema of ${component_name}. It will be imported as plain text`
      );
      return "text";
    }
  }
  ...
  async downloadAsset(url) {
    ...
    try {
      ...
      console.log(
        `Can't fetch asset's headers because of an error ${err.message}. Skipping this asset.`
      );
    }
  }
  ...
  async uploadAsset(asset) {
    ...
    try {
      ...
      const new_asset_request = await this.client.post(
        `spaces/${this.space_id}/assets`,
        new_asset_payload
      );
        ...
        return new Promise((resolve) => {
          form.submit(signed_request.post_url, (err) => {
            if (err) {
              ...
              this.assets.push({
                original_url: asset,
                new_url: signed_request.pretty_url,
                id: signed_request.id,
              });
              return resolve();
            }
          });
      ...
      if (
        err.config?.url === `/spaces/${this.space_id}/assets` &&
  ...
  async createFolders(folders) {
    ...
    for (let i = 0; i < folders.length; i++) {
      ...
      if (folders[i].parent) {
        const parent =
          folders.find((f) => f.path === folders[i].parent) ||
          links.folders.find((f) => compareSlugs(f.slug, folders[i].parent));
  ...
  async getUuidForTaxonomy(value, taxonomy, created_stories) {
    const created_story = created_stories.find(
      (s) => s.slug === value && s.content.component === taxonomy.taxonomy
    );
    if (created_story) {
  ...
  async createStories(stories, taxonomies) {
    ...
    for (let i = 0; i < stories.length; i++) {
      try {
        const content_type_taxonomies = taxonomies?.filter(
          (t) =>
            t.content_type === stories[i].content.component &&
            t.type === "relationship"
        );
        if (content_type_taxonomies.length) {
          for (
            let t_index = 0;
            t_index < content_type_taxonomies.length;
            t_index++
          ) {
            ...
            if (Array.isArray(story_field_value)) {
              for (let j = 0; j < story_field_value.length; j++) {
                story_field_value[j] = await this.getUuidForTaxonomy(
                  story_field_value[j],
                  t,
                  created_stories
                );
              ...
              stories[i].content[t.field] = await this.getUuidForTaxonomy(
                stories[i].content[t.field],
                t,
                created_stories
              );
            }
        ...
        const req = await this.client.post(`spaces/${this.space_id}/stories`, {
          story: stories[i],
        });
